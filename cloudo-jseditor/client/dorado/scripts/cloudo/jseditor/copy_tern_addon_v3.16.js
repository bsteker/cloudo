...
(function() {
  ...
  CodeMirror.TernServer.prototype = {
    ...
    jumpToDef: function(cm) { return jumpToDef(this, cm); },
  ...
  function hint(ts, cm, c) {
    ts.request(cm, {type: "completions", types: true}, function(error, data) {
      ...
      //copy//
      try {
        ts.request(cm,{"type":"doradoCompletions", types: true, from: from, to: to}, function(error, cdata){
          if (!error) {
            from = cdata.from;
            data.completions = [].concat(cdata.completions, data.completions);
          }
        });
      } catch (e) {}
  ...
  function showType(ts, cm) {
    ts.request(cm, "type", function(error, data) {
      ...
      if (ts.options.typeTip) {
       ...
       //copy//
        var dapiData;
		ts.request(cm, "doradoElementAPITip", function(error, cdata) {
	    	if ( error ) {
	    		console.log( error );
	    		return;
	    	}
		...
		if ( dapiData ) {
			cloudo.HINT.showDoradoElementAPITip( dapiData );
	    } else {
	    	tempTooltip(cm, tip);
	    }
      }
  ...
  function clearTip( cm, dom ) {
      if (!dom.parentNode) return;
      cm.off("cursorActivity", clearTip( cm, dom) );
      fadeOut(dom);
  }
  ...
  function jumpToDef(ts, cm) {
    ...
    function inner(varName) {
      ...
      ts.server.request(buildRequest(ts, doc, req), function(error, data) {
        ...
        //copy//
        showError(ts, cm, "不能找到变量的定义处.");
      });
    }
    if (!atInterestingExpression(cm))
    	//copy//
      dialog(cm, "跳转到一个变量", function(name) { if (name) inner(name);});
    else
      inner();
    return found;
  }
  ...
  function rename(ts, cm) {
    ...
    //copy//
    if (!/\w/.test(token.string)) showError(ts, cm, "光标所在处不是一个变量");
    dialog(cm, token.string + "重命名为"  , function(newName) {
